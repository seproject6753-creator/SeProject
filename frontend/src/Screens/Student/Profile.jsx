...
// Standalone fetch on this route so it doesn't depend on Home having loaded first
const Profile = () => {
  ...
  if (!profileData) {
    return <div className="p-8 text-slate-400">No profile data.</div>;
  }
      ...
      <div className="rounded-xl p-6 lg:p-8 mb-8 border border-white/5 shadow-xl" style={{backgroundImage:"linear-gradient(90deg, rgba(0,209,178,0.12), rgba(124,77,255,0.08))"}}>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            />
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold">{`${profileData.firstName || ''} ${profileData.middleName || ''} ${profileData.lastName || ''}`.trim()}</h1>
              <p className="text-slate-300 mt-1">SAP ID: {profileData.enrollmentNo}</p>
              <p className="text-slate-300">{profileData.branchId.name}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-xl p-6 border border-white/5 bg-slate-900/50 backdrop-blur">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Info label="Semester" value={profileData.semester} />
          </div>
        </div>
